...
public class IsoSkeleton implements PConstants {
	...
	public IsoSkeleton(PGraphics _canvas) {
		canvas = _canvas;
		adj = new ArrayList[0];
		VW = new float[0];
		nodes = new PVector[0];
	...
	private int overlapCheck(PVector test) {
		...
		for (int i = 0; i < nodes.length; i++) {
			PVector n = nodes[i];
			if (PApplet.abs(test.x - n.x) < tolerance && PApplet.abs(test.y - n.y) < tolerance
					&& PApplet.abs(test.z - n.z) < tolerance)
				return i;
		}
		return -1;
	}
	...
	private void polygonizeEnd(int v) {
		...
		for (int i = 0; i < adj[v].size(); i++) {
			...
			for (int j = 0; j < cap.length; j++)
				canvas.vertex(cap[j].x, cap[j].y, cap[j].z);
	...
	private void polygonizeElbow(int v) {
		...
		for (int i = 0; i < adj[v].size(); i++) {
			...
			if (otherNodeAvgCross.z < 0)
				otherNodeAvgCross.mult(-1);
	...
	private void plotHull(QuickHull3D hull) {
		...
		for (int i = 0; i < faceIndices.length; i++) {
			...
			for (int k = 0; k < faceIndices[i].length; k++) {
				...
				canvas.vertex((float) v.x, (float) v.y, (float) v.z);
			}
			canvas.endShape(CLOSE);
		}
	}
